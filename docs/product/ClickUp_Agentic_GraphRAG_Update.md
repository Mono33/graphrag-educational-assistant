...
| 11 | **Observability (Agent JSON Parse Hardening + LangSmith/Langfuse)** | LM, AG | 🟠 High | 4-6h | #6.6 (for 11a repro) | 🟡 IN PROGRESS — **11a** (JSON parse hardening, silent fallthrough fix) still TODO; **11b** (Langfuse) foundation **LANDED** by AG (Langfuse client + prompt management — `aix.domains.langfuse_prompts`, `scripts/ops/seed_langfuse_prompts.py`, `langfuse>=2.0.0`); 11b dashboard verification + cost tracking + sanity check still TODO |
| 9 | **Corrective RAG (Retrieval Grading)** | LM | 🔴 Urgent | 3-4h | None | TODO |
| 10 | **Conversation Memory (LangGraph Checkpointer + Multi-Turn Chat)** | LM | 🔴 Urgent | 5-7d | None | 🟢 LANDED 10.1-10.4 *(2026-05-01; regenerate via time-travel + CLI session REPL deferred to V2; #10.5 PostgresSaver migration tracked under CORE 4 #15)* |
...
**CORE 4 total effort:** ~13-18h (~2-2.5 days; #15 grew from 5h → 5-8h after the migration story was clarified to `AsyncSqliteSaver → AsyncPostgresSaver` + concurrent-request stress test on 2026-05-01)
...
L1 — Short-term thread memory: LangGraph AsyncSqliteSaver in dev → AsyncPostgresSaver in #15
...
| **10.5 — `AsyncPostgresSaver` migration** | Deferred to **CORE 4 #15** — see that subtask. 1-line API swap (`AsyncSqliteSaver` → `AsyncPostgresSaver(pool)`). Same schema shape, same code path, just durable + multi-instance. | rolls up to #15 | TODO | #10.3 |
...
**Depends on:** None
**Blocks:** **#15 (PostgresSaver upgrade)**, #16 (Long-Term Memory), #19 (Human-in-the-Loop Interrupts)
...
### Subtask 15: State Checkpointing (`AsyncSqliteSaver` → `AsyncPostgresSaver` migration)
**Priority:** 🟡 Medium | **Effort:** 5-8h | **Assignee:** LM
**Note:** First subtask of **CORE 4 — Personalization**. Depends on CORE 2 #10 already being live (`AsyncSqliteSaver`-backed, dev). Triggered alongside the broader app-wide SQLite → PostgreSQL migration in #6.6 P6 (Hetzner deploy).
