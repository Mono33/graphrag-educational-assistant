...
Dependency graph:
  ...
  #LAT-2  External-API Resilience (Semantic Scholar 429 + cache)  ← needs #11b
...
| LAT-2 | **External-API Resilience** *(Semantic Scholar 429 circuit breaker + per-run cache)* | promoted from #17.b | LM | 🟠 High | 2-3h | #11b | NEW *(2026-05-10 PM)* |
| LAT-3 | **Corrective-RAG Retry Hardening** *(skip trivial rewrites + Levenshtein guard)* | promoted from #17.b | LM | 🟡 Medium | 1-2h | #11b, #9 | NEW *(2026-05-10 PM)* |
| LAT-4 | **Parallel Retrieval Fan-out** *(KG + external concurrent — `asyncio.gather`)* | NEW | LM | 🟡 Medium | 2-3h | #11b | NEW *(2026-05-10 PM)* |
...
**CORE 3.5 total effort:** ~9-15h (~1.5-2 days) *(excluding #LAT-5 which is counted under CORE 3 #17 to avoid double-counting; including #LAT-5 the total is ~13-23h ≈ 2-3 days)*. Two of the six items (#LAT-1, #LAT-5) are already in the doc; the other four (#LAT-2, #LAT-3, #LAT-4, #LAT-6) are NEW and were surfaced by the 2026-05-10 PM live smokes.

**Latency budget snapshot — observed 2026-05-10 PM live smoke**

| Phase | KG-covered query *(ADHD, 1 attempt)* | Out-of-scope query *(fotosintesi, 2 attempts)* | Owned by |
|---|---|---|---|
| Plan | ~5s | ~5s | — *(low; no action)* |
| Retrieve attempt 1 | ~10-15s | **~30s** | `#LAT-2` *(Semantic Scholar 429 dominates the spike)* |
| Grade attempt 1 | ~3-5s | ~3-5s | — *(cheap LLM, OK)* |
| Retrieve attempt 2 | — | **~30s** | `#LAT-2` + `#LAT-3` *(only when corrective-RAG retries; skip-trivial-rewrite kills ~30-50% of these)* |
| Grade attempt 2 | — | ~3-5s | — |
| **Write** | **~100-110s** | **~97-109s** | **`#LAT-1`** *(single largest fixed cost — block-rendered, no streaming)* |
| Critic | ~15-20s | ~15-20s | — *(acceptable)* |
| **Total observed** | **~150s** | **~310s** | |
| **Target P50 (post-CORE-3.5)** | **< 90s** | **< 120s** | |

**Two single-biggest wins from this table:**
1. **`#LAT-1` Writer streaming** — same total time, dramatically different UX. 100s of "blank-card" waiting becomes 100s of progressive content rendering. Highest perceived-latency win even though it doesn't reduce the wall-clock number.
2. **`#LAT-2` Semantic Scholar 429 circuit breaker + per-run cache** — erases ~60s of pure waiting on hybrid queries (out-of-scope = the case where users wait the longest today). Highest wall-clock win.
