...
def test_retriever_backward_compat():
    ...
    try:
        ...
        from aix.agent.agents.retriever_agent import RetrievalResult, RetrieverAgent

        # Test 1: Create retriever with media enabled (default)
        RetrieverAgent(domain="neuro", enable_media_lookup=True)
        print("✅ RetrieverAgent created (media enabled)")

        # Test 2: Create retriever with media disabled
        RetrieverAgent(domain="neuro", enable_media_lookup=False)
        print("✅ RetrieverAgent created (media disabled)")
