...
class Text2CypherConverter:
    ...
    def _create_prompt_template(self, domain: str = None) -> ChatPromptTemplate:
        ...
        system_message = (
            """You are a Neo4j Cypher query expert for an educational knowledge graph system.
...
Convert the natural language question to a Cypher query. Return ONLY the Cypher query, no explanations."""
        ...
        return ChatPromptTemplate.from_messages(
            [("system", system_message), ("human", "Question: {question}\n\nCypher Query:")]
        )
...
"""
    ...
    def convert(self, question: str, domain: str = None) -> tuple[str, dict]:
        ...
        try:
            ...
            if cache_key not in Text2CypherConverter._schema_cache:
                logger.info(
                    f"[Text2Cypher] Schema cache MISS for domain='{cache_key}' — extracting (one-time cost)"
                )
                Text2CypherConverter._schema_cache[cache_key] = (
                    self.schema_extractor.extract_schema(domain=domain)
                )
                self.schema_info = Text2CypherConverter._schema_cache[cache_key]
                Text2CypherConverter._prompt_cache[cache_key] = self._create_prompt_template(
                    domain=domain
                )
    ...
    def _clean_cypher_query(self, raw_query: str) -> str:
        ...
        for line in lines:
            ...
            ):  # Skip any remaining code fences
    ...
    def _repair_udl_query(self, query: str) -> str:
        ...
        query = re.sub(
            r'WHERE\s+m\.name\s*=\s*"[Mm]athematics?"\s+AND\s+', "WHERE ", query
        )  # ✅ Added
        ...
        query = re.sub(
            r'toLower\("([^"]+)"\s+(RETURN|AND|OR|LIMIT|WHERE)\b', r'toLower("\1") \2', query
        )
        ...
        query = re.sub(
            r'CONTAINS toLower\("([^"]+)"\s+RETURN', r'CONTAINS toLower("\1") RETURN', query
        )
        ...
        query = re.sub(
            r'AND toLower\([^)]+\) CONTAINS toLower\("([^"]+)"\s+RETURN',
            r'AND toLower(s.name) CONTAINS toLower("\1") RETURN',
            query,
        )
        ...
        query = re.sub(
            r'CONTAINS toLower\("([^"]+)"\s+AND\s+', r'CONTAINS toLower("\1") AND ', query
        )
        ...
        # Pattern: CONTAINS toLower("term" AND -> CONTAINS toLower("term") AND
        query = re.sub(
            r'CONTAINS\s+toLower\("([^"]+)"\s+(AND|RETURN)', r'CONTAINS toLower("\1") \2', query
        )
        ...
        if re.search(r"\(\s*m\s*:\s*PedagogicalMethodology\s*\)", query) and not re.search(
            r"\(\s*p\s*:", query
        ):
        ...
        if re.search(r"\(\s*lr\s*:\s*LearningResource\s*\)", query) and not re.search(
            r"\(\s*p\s*:", query
        ):
        ...
        if re.search(
            r'toLower\(s\.name\)\s*CONTAINS\s*toLower\("cognitive disability"\)',
            query,
            flags=re.IGNORECASE,
        ):
            # Remove " OR s.name = "<anything with Cognitive ...>"
            query = re.sub(
                r'\s*OR\s*s\.name\s*=\s*"[^"]*(?i:cognitive)[^"]*"', "", query, flags=re.IGNORECASE
            )
            # Also remove accidental concatenation like ...CONTAINS("...") s.name = ...
            query = re.sub(
                r'\)\s*s\.name\s*=\s*"[^"]*(?i:cognitive)[^"]*"', ")", query, flags=re.IGNORECASE
            )
    ...
    def _detect_udl_contamination(self, query: str, domain: str) -> bool:
        ...
        for label in UDL_ONLY_LABELS:
            if f":{label}" in query:
                logger.warning(
                    f"[UDL Contamination] Label '{label}' detected in Neuro query - this may return 0 results"
                )
    ...
    def _repair_neuro_query(self, query: str) -> str:
        ...
        if re.search(conflict_pattern, query):
            ...
            logger.info(
                f"Fixing variable name conflict in relationship query (old var: '{old_var}')"
            )

            # Step 1: Replace the conflicting pattern in MATCH clause
            # [r:REL]->(r:Label) → [rel:REL]->(target:Label)
            query = re.sub(
                r"\[(\w+):([^\]]+)\]->\((\1):(\w+)([^)]*)\)", r"[rel:\2]->(target:\4\5)", query
            )
            ...
            if re.search(return_pattern, query):
                ...
                query = re.sub(
                    rf"\blabels\({old_var}\)\s+as\s+target_labels\b",
            ...
            if len(parts) >= 2:
                ...
                if all_simple:
                    ...
                    if len(set(var_names)) > 1:  # Only log if variables differ
                        logger.info(
                            f"Fixing {len(parts)}-way UNION column names: {', '.join(var_names)} → 'concept'"
                        )
                ...
                # MATCH (...)-[r:REL]->(...) RETURN ..., type(r), ..., labels(...) as source_labels, labels(...) as target_labels
                    ...
                    all_relationship = all(
                        re.search(relationship_union_pattern, part.strip()) for part in parts
                    )
                    ...
                    if all_relationship:
                        ...
                        if len(set(var_pairs)) > 1:  # Only log if variables differ
                            logger.info(
                                f"Fixing {len(parts)}-way relationship UNION: {', '.join(var_pairs)} → 'source/target'"
                            )
                    ...
                    # Fix: Add AS concept to the node variable
                        ...
                        all_type_var = all(
                            re.search(type_var_pattern, part.strip(), re.IGNORECASE)
                            for part in parts
                        )
                        ...
                        if all_type_var:
                            ...
                            if len(set(var_names)) > 1:  # Only log if variables differ
                                logger.info(
                                    f"[Q2 Fix] Standardizing {len(parts)}-way UNION with type column: {', '.join(var_names)} → 'concept'"
                                )
        ...
        if re.search(r"MATCH\s+\([^)]+\),\s*\([^)]+\)\s+RETURN", query):
            ...
            for label1, label2, type1, type2 in comparison_pairs:
                ...
                if match:
                    ...
                    logger.info(
                        f"Converted Cartesian product to UNION query for {label1} vs {label2}"
                    )
                    return union_query
        ...
        if re.search(
            r"\b(how|why|does|affects?|influences?|impacts?|supports?|enhances?|leads?\sto)\b",
            query,
            re.IGNORECASE,
        ):
            ...
            if not re.search(r"-\[", query):
                logger.warning(
                    "Query asks 'how/why' but has no relationship pattern - may return incomplete results"
                )
        ...
        if re.search(r"\bUNION\b", query, re.IGNORECASE):
            ...
            if num_branches >= 2:
                ...
                if trailing_limit and not other_have_limit:
                    ...
                    branches[-1] = (
                        last_branch[: trailing_limit.start()].strip() + f" LIMIT {per_branch}"
                    )
...
class Text2CypherPipeline:
    ...
    def __init__(
        ...
        self.converter = Text2CypherConverter(
            neo4j_uri, neo4j_user, neo4j_password, openai_api_key, model, base_url
        )
    ...
    def process_question(self, question: str, domain: str = None, execute: bool = True) -> dict:
        ...
        if execute and metadata.get("is_valid", False):
            ...
            if invalid_labels and not results and not execution_error:
                ...
                search_terms = [
                    _re.sub(r"(?<!^)(?=[A-Z])", " ", lbl).lower().strip() for lbl in invalid_labels
                ]
                conditions = " OR ".join([f'toLower(n.name) CONTAINS "{t}"' for t in search_terms])
                name_fallback = (
            ...
            if not results and not execution_error:
                ...
                widened_query = re.sub(
                    r" s\.name = ", " toLower(s.name) CONTAINS toLower(", widened_query
                )
                widened_query = re.sub(
                    r" lr\.category = ", " toLower(lr.category) CONTAINS toLower(", widened_query
                )
                ...
                if not results and not execution_error and "-[" in cypher_query:
                    ...
                    if fallback_query:
                        logger.info(
                            "[Fallback] Relationship query returned 0 results, trying definition fallback"
                        )
                        fallback_results, fallback_error = self.converter.execute_query(
                            fallback_query
                        )
                        ...
                        if fallback_results:
                            ...
                            logger.info(
                                f"[Fallback] Definition fallback returned {len(results)} results"
                            )
