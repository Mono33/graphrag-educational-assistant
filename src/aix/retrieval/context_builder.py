...
class MethodologyRanker:
    ...
    def rank_methodologies(
        self, nodes: list[dict], query_metadata: dict
        ...
        for i, node in enumerate(deduped_nodes):
            logger.info(
                f"[DEBUG] Node {i + 1}: name='{node.get('name', 'N/A')}', labels={node.get('labels', [])}, keys={list(node.keys())}"
            )
        ...
        if needs_balancing:
            logger.info(
                "[Smart Ranking] Preserving interleaved order for balanced primary/supporting split"
            )
        else:
    ...
    def _deduplicate_nodes(self, nodes: list[dict]) -> list[dict]:
        ...
        if removed > 0:
            logger.info(
                f"[Dedup] Merged {len(nodes)} → {len(deduped)} nodes ({removed} duplicates removed)"
            )
    ...
    _RELATIONSHIP_NAMES = frozenset(
        {"SUGGESTS", "NO_SUGGESTS", "MITIGATED_BY", "RELATED_TO", "PART_OF"}
    )
    ...
    def _create_recommendation(
        self, node: dict, query_metadata: dict
    ) -> Optional[MethodologyRecommendation]:
        ...
        implementation = kb_info.get(
            "implementation", node_description or f"Apply {name} with appropriate adaptations"
        )
        ...
        if not applications:
            applications = [
                f"Consultare il contesto specifico della classe per applicazioni pratiche di {name}"
            ]

        # --- Special considerations: kb_info > generic ---
        special_considerations = kb_info.get(
            "special_needs_adaptations", ["Adapt based on individual student needs"]
        )
    ...
    def _calculate_confidence(
        self, relevance_score: float, evidence_type: str, kb_info: dict
    ) -> ConfidenceLevel:
    ...
    def _detect_query_intent(self, query: str) -> dict:
        ...
        if is_comparison:
            logger.info(
                f"[Intent Detection] Comparison query detected (keywords: {found_keywords})"
            )

        return {"is_comparison": is_comparison, "comparison_keywords": found_keywords}
    ...
    def _apply_dynamic_balancing(self, nodes: list[dict], target_size: int = 15) -> list[dict]:
        ...
        if len(top_labels) < 2:
            ...
            logger.info("[Smart Ranking] Only 1 label type, using standard ranking")
    ...
    def _detect_dominant_labels(
        self, distribution: dict[str, int], threshold: float = 0.7
    ) -> list[str]:
        """
    ...
    def _balance_by_labels(
        self, nodes: list[dict], distribution: dict[str, int], target_size: int = 15
    ) -> list[dict]:
        """
        ...
        if len(top_labels) == 2:
            ...
            logger.info("[Smart Ranking] 2 labels detected → 7/7/1 split")
        elif len(top_labels) == 3:
            slots_per_label = [5, 5, 5]  # 5+5+5 = 15
            logger.info("[Smart Ranking] 3 labels detected → 5/5/5 split")
        else:
        ...
        for i, label in enumerate(top_labels[: len(slots_per_label)]):
            ...
            label_nodes = sorted(
                nodes_by_label[label], key=lambda n: n.get("rank_score", 0), reverse=True
            )
...
class EvidenceSynthesizer:
    ...
    def _analyze_vector_similarities(self, nodes: list[dict]) -> str:
        ...
        avg_similarity = sum(n.get("vector_similarity", 0) for n in vector_nodes) / len(
            vector_nodes
        )
...
class EducationalContextBuilder:
    ...
    async def build_context(
        ...
        try:
            ...
            supporting_methodologies = all_recommendations[
                max_primary : max_primary + max_supporting
            ]
            ...
            logger.info(
                f"Context built successfully with {len(primary_methodologies)} primary recommendations"
            )
            return context
    ...
    def _build_student_profile(
        self, query: str, metadata: dict, nodes: list[dict]
    ) -> StudentProfile:
        ...
        if any(term in query_lower for term in ["special", "disabilità", "difficoltà", "bisogni"]):
            ...
            re.search(r"\b" + re.escape(term), query_lower)
            for term in ["verific", "valut", "test", "esam", "quiz", "interroga"]
        ):
    ...
    def _determine_implementation_priority(
        self, methodologies: list[MethodologyRecommendation], student_profile: StudentProfile
    ) -> list[str]:
        ...
        high_confidence = [
            ...
            if m.confidence in [ConfidenceLevel.VERY_HIGH, ConfidenceLevel.HIGH]
        ]
        if high_confidence:
    ...
    def _calculate_overall_confidence(
        self, recommendations: list[MethodologyRecommendation], triple_count: int, metadata: dict
    ) -> ConfidenceLevel:
        ...
        avg_confidence = sum(confidence_scores[r.confidence] for r in recommendations) / len(
            recommendations
        )
    ...
    def _get_fallback_strategies(
        self, confidence: ConfidenceLevel, student_profile: StudentProfile, metadata: dict = None
    ) -> list[str]:
