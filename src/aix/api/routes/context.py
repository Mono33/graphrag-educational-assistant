...
from aix.retrieval.context_builder import ConfidenceLevel as CBConfidenceLevel
...
def _build_methodology_explainability(
    ...
    denom = (semantic_score if semantic_score else 1.0) * (
        vector_similarity if vector_similarity else 1.0
    )
    ...
    if hop == 0:
        ...
        reasoning = "Direct match from Cypher query (0 hops)"
        ...
        phrase = (
            "Strategia individuata per similarità strutturale nel grafo"
        ...
        phrase = (
            "Strategia individuata per similarità semantica con i concetti del Knowledge Graph"
            + (f" (similarità: {score_pct})" if score_pct else "")
            + f". Confidenza: {confidence_it}."
        )
...
def _build_explainability_summary(
    retrieval_result, nodes: list, n_methods: int = 0
) -> ExplainabilitySummary:
    """
    ...
    semantic = sum(
        1
        for n in nodes
        if (n.get("retrieval_stage") or "")
        in ("semantic_search", "keyword_semantic", "vector_neighbor")
    )
    ...
    return ExplainabilitySummary(
        ...
        retrieval_phases={
            "graph_traversal": RetrievalPhase(
                nodes_found=graph_count, time_ms=_ms("graph_traversal")
            ),
            "semantic_search": RetrievalPhase(
                nodes_found=semantic_count, time_ms=_ms("semantic_search")
            ),
        ...
        knowledge_graph_stats=KGStats(
            ...
            total_relationships=len(retrieval_result.triples)
...
def _build_concept_graph(
    nodes: list, triples: list, max_nodes: int = 20, max_edges: int = 30
) -> ConceptGraph:
    """
    ...
    for method in methodologies[:max_count]:
        result.append(
            MethodologyInfo(
                ...
                classroom_applications=method.classroom_applications[:3]
                if method.classroom_applications
                else [],
                special_considerations=method.special_considerations[:3]
...
def _format_prompt_section(
    ...
    if language in ("it", "italian"):
        ...
        lines.extend(["", "### Metodologie Raccomandate dal Knowledge Graph"])
        ...
        if context.supporting_methodologies:
            ...
            for method in context.supporting_methodologies:
                ...
                lines.append(
                    f"  - Rilevanza: {method.relevance_score:.2f} | Evidenza: {method.evidence_type}"
                )
        ...
        if context.confidence_level.value in ["low", "very_low"]:
            lines.append("")
            lines.append(
                "**Nota**: Il livello di confidenza è basso. Si consiglia di consultare uno specialista per raccomandazioni personalizzate."
            )
        ...
        lines.extend(["", "### Recommended Methodologies from Knowledge Graph"])
        ...
        if context.supporting_methodologies:
            ...
            for method in context.supporting_methodologies:
                lines.append(f"\n- **{method.name}** ({method.category})")
                lines.append(
                    f"  - Relevance: {method.relevance_score:.2f} | Evidence: {method.evidence_type}"
                )
                if method.implementation_guidance:
                    lines.append(f"  - Implementation: {method.implementation_guidance}")
                if method.classroom_applications:
        ...
        if context.confidence_level.value in ["low", "very_low"]:
            lines.append("")
            lines.append(
                "**Note**: Confidence is low. Consider consulting a specialist for personalized recommendations."
            )
...
async def get_context(
    ...
    *Note:* Agent mode (LangGraph multi-agent lesson planning) is exposed by separate
    ...
    try:
        logger.info(
            f"Processing context request: domain={request.domain}, query={request.query[:50]}..."
        )
        ...
        processor = EnhancedMultilingualText2Cypher(use_vectors=True, domain=domain)

        # Process the query through the full pipeline
        result = await processor.process_query_with_retrieval(
            query=request.query, domain=domain, max_methodologies=request.max_methodologies
        )
        ...
        if educational_context and hasattr(educational_context, "metadata"):
            ...
            logger.info(
                f"[API] Using educational_context metadata: nodes={total_nodes_count}, relationships={total_relationships_count}"
            )
        else:
            ...
            logger.info(
                f"[API] Using retrieval_result metadata: nodes={total_nodes_count}, relationships={total_relationships_count}"
            )
        ...
        context_data = ContextData(
            ...
            primary_methodologies=_format_methodologies(
                educational_context.primary_methodologies, request.max_methodologies
            ),
            supporting_methodologies=_format_methodologies(
                educational_context.supporting_methodologies, max(2, request.max_methodologies // 2)
            ),
        ...
        if request.include_explainability and retrieval_result:
            ...
            for methodology in (
                ...
                if node:
                    ...
                    methodology.explainability_phrase = (
                        f"Metodologia estratta dal Knowledge Graph. Confidenza: {confidence_it}."
                    )
            ...
            n_methods = len(context_data.primary_methodologies) + len(
                context_data.supporting_methodologies
            )
            explainability_summary = _build_explainability_summary(
                retrieval_result, nodes, n_methods
            )
        ...
        n_methods_total = len(context_data.primary_methodologies) + len(
            context_data.supporting_methodologies
        )
        context_warning = _build_context_warning(
            kg_data_available, overall_confidence, n_methods_total
        )
            ...
            raw_nodes = [
                RawNode(
                    ...
                    properties={
                        ...
                        if k not in ["id", "name", "labels", "category", "description"]
                    },
                )
        ...
        domain_prompt_ctx = _build_domain_prompt_context(
            ...
            if retrieval_result and hasattr(retrieval_result, "triples")
            else None,
        )
