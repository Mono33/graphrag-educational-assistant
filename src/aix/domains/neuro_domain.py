...
class NeuroDomainConfig(BaseDomainConfig):
    ...
    def get_valid_methodology_labels(self) -> list[str]:
        ...
        return [
            ...
            "AffectiveMotivationalProcesses",  # Correct label names
            # =====================================================
            # NEUROPLASTICITY & BRAIN
            # =====================================================
            "BrainAdaptability",
            "Neuroplasticity",
            "Brainfunction",  # Note: lowercase 'f'
            # =====================================================
            ...
            # =====================================================
            "LearningStrategies",
            "Learningstrategies",  # Both case variants
    ...
    # ============================================================
    # RETRIEVAL BOOSTS (from aix.retrieval.graph_retriever.py)
    # ============================================================
    ...
    # ============================================================
    # TEXT2CYPHER EXAMPLES (from aix.retrieval.text2cypher.py lines 303-476)
    # ============================================================
...
"""
    ...
    def repair_cypher_query(self, query: str) -> str:
        """Neuro-specific Cypher repair logic (from aix.retrieval.text2cypher.py lines 801-1028)"""
    ...
    def _repair_assessment_queries(self, query: str) -> str:
        ...
        for term in assessment_terms:
            ...
            if re.search(wrong_pattern, query, re.IGNORECASE):
                ...
                query = re.sub(
                    wrong_pattern, r'(\1:Assessment {domain: "neuro"})', query, flags=re.IGNORECASE
                )
                import logging

                logging.getLogger(__name__).info(
                    "[Assessment Fix] Replaced TeachingPractices search with Assessment label"
                )

        return query

    # ============================================================
    # MULTILINGUAL TERMS (from aix.retrieval.multilingual_text2cypher.py lines 192-263)
    # ============================================================
    ...
    # ============================================================
    # LLM CHAIN PROMPTS (from aix.generation.llm_chain.py lines 53-56)
    # ============================================================

    def get_system_prompt(self) -> str:
        """Fetch the neuro system prompt from Langfuse (legacy GraphRAG mode)."""
        from aix.domains.langfuse_prompts import fetch_prompt

        return fetch_prompt("neuro.system_prompt")

    def get_writer_prompt(self) -> str:
        """Fetch the neuro writer prompt from Langfuse (agent mode)."""
        from aix.domains.langfuse_prompts import fetch_prompt

        return fetch_prompt("neuro.writer_prompt")

    def get_response_template(self) -> str:
        """Fetch the neuro response template from Langfuse (legacy GraphRAG mode)."""
        from aix.domains.langfuse_prompts import fetch_prompt

        return fetch_prompt("neuro.response_template")

    # ============================================================
    # CONTEXT BUILDER - METHODOLOGY CATEGORIES
    # (from aix.retrieval.context_builder.py lines 132-208)
    # ============================================================
    ...
    # (from aix.retrieval.context_builder.py lines 231-240)
    # ============================================================
    ...
    def get_special_needs_mapping(self) -> dict:
        ...
        return {
            ...
            "WorkingMemory": [
                ...
                "chunking_strategies",
                "rehearsal_techniques",
            ],
            "ExecutiveFunctions": [
                "planning_support",
                "organization_scaffolding",
                "self_regulation_teaching",
            ],
            ...
            "GrowthMindset": [
                "effort_praise",
                "challenge_framing",
                "mistake_learning_opportunities",
            ],
        }
