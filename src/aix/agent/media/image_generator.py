...
class ImageGenerator:
    ...
    def __init__(self, cache_dir: Optional[str] = None, enable_caching: bool = True):
        """
    ...
    async def generate_educational_diagram(
        ...
        prompt_template = DIAGRAM_PROMPTS.get(
            diagram_type, DIAGRAM_PROMPTS[DiagramType.CONCEPT_MAP]
        )
    ...
    async def generate_for_concepts(
        ...
        for i, concept in enumerate(concepts[:max_images]):
            ...
            result = await self.generate_educational_diagram(
                concept=name, description=description, diagram_type=diagram_type
            )
