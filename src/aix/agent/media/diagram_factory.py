...
from .image_generator import DiagramType as DalleDiagramType
...
class DiagramFactory:
    ...
    def __init__(self):
        ...
        self._generation_count = {"mermaid": 0, "dalle": 0, "canva": 0}
    ...
    async def generate(
        ...
        try:
            if generator_type == GeneratorType.MERMAID:
                return await self._generate_mermaid(concept, mermaid_type, related_concepts)
            elif generator_type == GeneratorType.DALLE:
                return await self._generate_dalle(concept, dalle_type, description)
            elif generator_type == GeneratorType.CANVA:
                return await self._generate_canva(concept, diagram_type, related_concepts)
            else:
    ...
    async def _generate_mermaid(
        self, concept: str, diagram_type: str, related_concepts: Optional[list[str]]
    ) -> DiagramResult:
        """Generate diagram using Mermaid.js"""
        result = await self.mermaid.generate(
            concept=concept, diagram_type=diagram_type, related_concepts=related_concepts
        )
    ...
    async def _generate_dalle(
        self, concept: str, diagram_type: str, description: Optional[str]
    ) -> DiagramResult:
