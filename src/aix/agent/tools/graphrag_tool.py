...
class GraphRAGTool:
    ...
    def _ensure_initialized(self):
        ...
        try:
            ...
            # Use the retriever from aix.retrieval.text2cypher (it has the Neo4j driver already)
            self._retriever = self._text2cypher.graph_retriever
            self._context_builder = EducationalContextBuilder(domain=self.domain)

            self._initialized = True
            logger.info("[GraphRAGTool] Initialization complete")
    ...
    async def search(self, query: str) -> GraphRAGResult:
        ...
        try:
            ...
            result_dict = await self._text2cypher.process_query_with_retrieval(
            ...
            for triple in triples:
                if isinstance(triple, dict):
                    ...
                    relationships.append(
                        {"source": str(triple[0]), "type": str(triple[1]), "target": str(triple[2])}
                    )
